...
const SPEC_FILE = path.join(__dirname, 'backend-linux.spec');

console.log('🔧 Preparing Backend for Linux...');
...
try {
  ...
  console.error('Install: pip3 install pyinstaller');
  process.exit(1);
}
...
const builtExe = path.join(__dirname, '..', 'dist', 'server');
if (fs.existsSync(builtExe)) {
  fs.copyFileSync(builtExe, path.join(BACKEND_DEST, 'server'));
  // Make executable
  fs.chmodSync(path.join(BACKEND_DEST, 'server'), '755');
